...
### 2.3、服务器 CLI 模式（不使用 Electron / 不需要 GUI 环境）

如果你只是想在服务器、Docker 容器、SSH 会话里启动代理服务，可以直接使用 CLI 模式。

- 这个模式**不会**设置系统代理
- 这个模式**不会**弹出 GUI，也**不依赖** Electron 桌面环境
- 这个模式默认按**默认模式**启动：开启请求拦截、开启远程配置、关闭系统代理、关闭证书安装
- 启动后只会监听本地代理服务，并在控制台输出地址、端口和运行日志
...
`pnpm server -- --host 0.0.0.0 --port 31181`

启动成功后，控制台会看到类似：

- `代理服务器地址: http://127.0.0.1:31181`
- `代理服务器地址: https://127.0.0.1:31181`
- 以及代理启动、DNS 优选、测速、请求拦截相关日志

#### 构建说明

CLI 模式本身是纯 Node.js 脚本，不需要 Electron 打包步骤。一般只需要：

1. `pnpm install`
2. `pnpm server`

如果你只想在服务器上部署 CLI，通常连 GUI 构建都不需要。
